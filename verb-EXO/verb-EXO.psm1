    ...
    * 9:47 AM 6/9/2021 init; flipped -displayname to -identifier, and handle smtpaddr|alias|displayname lookups 
    ...
    BEGIN {
        ...
        if(!$Identifier -AND (gcm get-clipboard) -AND (get-clipboard)){
            ...
            write-warning "$((get-date).ToString('HH:mm:ss')):No Identifier specified, and clipboard did not match 'Identifier' content" ; 
            Break ;
        } ; 
    ...
    PROCESS {
        ...
        write-verbose "$((get-date).ToString('HH:mm:ss')):$((get-alias ps1GetxUser).definition) w`n$(($pltGetxUser|out-string).trim())" ;         
        #rxo ; cmsol ; caad ; rx10 ;
        $error.clear() ;
        TRY {
            ...
            if($msolu = get-msoluser -user $txUser.UserPrincipalName |?{$_.islicensed}){
                ...
                if($taadu.extensionproperty.onPremisesDistinguishedName -match $rgxCMWDomain){
                    $bCmwAD=$true ;
                    write-warning "$((get-date).ToString('HH:mm:ss')):ADUser is onprem CMW hybrid!:`n$($taadu.extensionproperty.onPremisesDistinguishedName)" ; 
                ...
                write-warning "$((get-date).ToString('HH:mm:ss')):No matching licensed MSolu:(get-msoluser -user $txUser.UserPrincipalName)" ; 
            } ; 
        } CATCH {
            $ErrTrapd=$Error[0] ;
            $smsg = "Failed processing $($ErrTrapd.Exception.ItemName). `nError Message: $($ErrTrapd.Exception.Message)`nError Details: $($ErrTrapd)" ;
        ...
        if($txUser){
            ...
            "Licenses Assigned:`n$((($msolu.licenses.AccountSkuId) -join ";" | out-string).trim())" ;
                ...
                write-warning "$((get-date).ToString('HH:mm:ss')):Unable to find matching MsolU for $Identifier" ; 
            } ; 
        } ; 
