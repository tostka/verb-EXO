            ...
            if($Msgs){
                ...
                if($mFails = $msgs | ?{$_.status -eq 'Failed'} | select -first $MessageTraceDetailLimit){
                    ...
                    foreach($failed in $mFails){
                        ...
                        if($failed | ?{$_.Subject -match '^Automatic\sreply:\s'}){
                            $FailMsgSummary.FailCode += @('FailOOO') ; 
                        } ; 
                        if($failed | ?{$_.Subject -match '^Recall:\s'}){
                            $FailMsgSummary.FailCode += @('FailRecall') ; 
                        } ; 
                        if($failed | ?{$_.Subject -notmatch '^Recall:\s' -AND $_.Subject -notmatch '^Automatic\sreply:\s'}){
                            ...
                            if($FODetail | ?{$_.event -eq 'Transport rule' -AND $_.Detail -match 'OtherAccts-External-Mail-Rejection'}){
                                $FailMsgSummary.FailCode += @('FailOtherAcctBlock') ; 
                            } ; 
                            if($FODetail | ?{$_.event -eq 'Transport rule' -AND $_.Detail -match 'ConfRm-External-Mail-Rejection'}){
                                $FailMsgSummary.FailCode += @('FailConfRmExtBlock') ; 
                            } ; 
                            if($FODetail | ?{$_.event -eq 'Transport rule' -AND $_.Detail -match '^Security(\s-\s|-)'}){
...
$(
    switch -regex ($FV){
        'FailOtherAcctBlock'{
            "`n$($FV): Blocked by Security-mandated Transport rule:'OtherAccts-External-Mail-Rejection'`n"
        }
        'FailConfRmExtBlock'{
            "`n$($FV): Blocked by Security-mandated Transport rule:'ConfRm-External-Mail-Rejection'`n"
        }
        'FailOOO'{
            "`n$($FV): Blocked by Security Policy: Blocked external delivery of Out-Of-Office messages`n"
        }
        'FailRecall' {
...
"@ ; 
                    ...
                    if($mOOO = $mFails | ?{$_.Subject -match '^Automatic\sreply:\s'}){
                        $smsg = "adding:`$hReports.MsgsFailOOO" ; 
                        if ($logging) { Write-Log -LogContent $smsg -Path $logfile -useHost -Level Info } #Error|Warn|Debug 
                        else{ write-host -foregroundcolor yellow "$((get-date).ToString('HH:mm:ss')):$($smsg)" } ; 
                        $hReports.add('MsgsFailOOO',$mOOO) ; 
                    ...
                    if($mRecl = $mFails | ?{$_.Subject -match '^Recall:\s'}){
                        $smsg = "adding:`$hReports.MsgsFailRecall" ; 
                        if ($logging) { Write-Log -LogContent $smsg -Path $logfile -useHost -Level Info } #Error|Warn|Debug 
                        else{ write-host -foregroundcolor yellow "$((get-date).ToString('HH:mm:ss')):$($smsg)" } ; 
                        $hReports.add('MsgsFailRecall',$mRecl) ; 
                        $smsg = $sBnr3="`n#*~~~~~~v Status FAILED: Expected: Recalled message v~~~~~~" ;
                        $smsg += "`n$($mRecl| measure | select -expand count) msgs:Expected Sender Recalled Message `n$(($mRecl| ft -a $prpGXMTfta |out-string).trim())" ;
                        $smsg += "`n$($sBnr3.replace('~v','~^').replace('v~','^~'))`n"  ;
                        if ($logging) { Write-Log -LogContent $smsg -Path $logfile -useHost -Level Info } #Error|Warn|Debug 
                        else{ write-host -foregroundcolor blue "$((get-date).ToString('HH:mm:ss')):$($smsg)" } ; 
                    } ;
                    if($mFailsOther = $mFails | ?{$_.Subject -notmatch '^Recall:\s' -AND $_.Subject -notmatch '^Automatic\sreply:\s'}){
                        $smsg = "adding:`$hReports.MsgsFailOther" ; 
                        if ($logging) { Write-Log -LogContent $smsg -Path $logfile -useHost -Level Info } #Error|Warn|Debug 
                        else{ write-host -foregroundcolor yellow "$((get-date).ToString('HH:mm:ss')):$($smsg)" } ; 
                        $hReports.add('MsgsFailOther',$mFailsOther) ; 
                        $smsg = $sBnr3="`n#*~~~~~~v Status FAILED: Other Failure message v~~~~~~" ;
                        $smsg += "`n$(($mFailsOther | ft -a |out-string).trim())" ;
                        $smsg += "`n$($sBnr3.replace('~v','~^').replace('v~','^~'))`n"  ;
                        if ($logging) { Write-Log -LogContent $smsg -Path $logfile -useHost -Level Info } #Error|Warn|Debug 
                        else{ write-host -foregroundcolor blue "$((get-date).ToString('HH:mm:ss')):$($smsg)" } ; 

                        if($DetailedOtherFails){
                            $aggDetailedOtherFails = @() ; 
                            $smsg = "-DetailedOtherFails: running Get-xoMessageTraceDetail on Other Fails (to MessageTraceDetailLimit:$($MessageTraceDetailLimit))" ; 
                            if ($logging) { Write-Log -LogContent $smsg -Path $logfile -useHost -Level Info } 
                            else{ write-host -foregroundcolor green "$((get-date).ToString('HH:mm:ss')):$($smsg)" } ;
                            #Levels:Error|Warn|Info|H1|H2|H3|H4|H5|Debug|Verbose|Prompt|Success
                            $mfNum = 0 ; 
                            foreach($mf in ($mFailsOther | select -first $MessageTraceDetailLimit)){
                                $mfNum++ ; 
                                $smsg = $sBnr4="`n#*``````v Other Fail#$($mfNum) : v``````" ; 
                                if ($logging) { Write-Log -LogContent $smsg -Path $logfile -useHost -Level H4 } else{ write-host -foregroundcolor green "$((get-date).ToString('HH:mm:ss')):$($smsg)" } ;
                                $smsg = ":`n$(($mf|out-string).trim())" ; 
                                if ($logging) { Write-Log -LogContent $smsg -Path $logfile -useHost -Level Info } #Error|Warn|Debug 
                                else{ write-host -foregroundcolor green "$((get-date).ToString('HH:mm:ss')):$($smsg)" } ;
                                TRY{
                                    $FODetail =  $mf | Get-xoMessageTraceDetail -ea STOP; 
                                    $aggDetailedOtherFails += $FODetail ; 
                                    $smsg = "Matching: Get-xoMessageTraceDetail:`n$(($FODetail | out-string).trim())" ; 
                                    if ($logging) { Write-Log -LogContent $smsg -Path $logfile -useHost -Level WARN -Indent} 
                                    else{ write-WARNING "$((get-date).ToString('HH:mm:ss')):$($smsg)" } ; 

                                    if($FODetail | ?{$_.event -eq 'Transport rule' -AND $_.Detail -match 'OtherAccts-External-Mail-Rejection'}){
                                        $smsg = $sBnr3="`n#*~~~~~~^ Status FAILED: Expected: Security Policy Block (Other Accounts OU Mailbox Mail Block) ^~~~~~~`n" ;
                                        if ($logging) { Write-Log -LogContent $smsg -Path $logfile -useHost -Level WARN -Indent} 
                                        else{ write-WARNING "$((get-date).ToString('HH:mm:ss')):$($smsg)" } ; 
                                    } ; 
                                    if($FODetail | ?{$_.event -eq 'FAIL' -AND $_.Detail -match 'Hop\scount\sexceeded\s-\spossible\smail\sloop'}){
                                        $smsg = $sBnr3="`n#*~~~~~~^ Status FAILED: Expected: Mail Loop (unresolvable incomplete offboarded user) ^~~~~~~`n" ;
                                        $smsg += "`n (Likely license removed => deleted target mailbox, left ADUser and OnPrem Recipient objects)"
                                        if ($logging) { Write-Log -LogContent $smsg -Path $logfile -useHost -Level WARN -Indent} 
                                        else{ write-WARNING "$((get-date).ToString('HH:mm:ss')):$($smsg)" } ; 

                                        $smsg = "Checking function of problem recipient in ExchangeOnline and ExchangeOnprem" ; 
                                        if ($logging) { Write-Log -LogContent $smsg -Path $logfile -useHost -Level Info } 
                                        else{ write-host -foregroundcolor green "$((get-date).ToString('HH:mm:ss')):$($smsg)" } ;
                                        ...
                                        $xopRcp = get-recipient $mf.RecipientAddress -ea 0;
                                        $xoRCP = get-xorecipient $mf.RecipientAddress -ea 0 ; 
                                        $adu = get-aduser -id $xoRCP.alias -ea 0 ; 

                                        if($xopRcp.RecipientTypeDetails -eq 'RemoteUserMailbox'){
                                            $smsg = "$($xopRcp.primarysmtpaddress) onprem is NOT a functional recipient for delivery (RemoteUserMailbox:forwards all traffic to cloud)" ; 
                                            if ($logging) { Write-Log -LogContent $smsg -Path $logfile -useHost -Level WARN -Indent} 
                                            else{ write-WARNING "$((get-date).ToString('HH:mm:ss')):$($smsg)" } ; 
                                            if($xoRCP.RecipientTypeDetails -eq 'MailUser'){
                                                $smsg = "$($xoRCP.primarysmtpaddress) cloud recipient is NOT a functional recipient for delivery (MailUser:forwards all traffic to OnPrem mail system)" ; 
                                                if ($logging) { Write-Log -LogContent $smsg -Path $logfile -useHost -Level WARN -Indent} 
                                                else{ write-WARNING "$((get-date).ToString('HH:mm:ss')):$($smsg)" } ; 
                                            }
                                            if($adu.DistinguishedName -match ',OU=Disabled,' -OR $adu.Enabled -eq $false ){
                                                $smsg = "CONFIRMED:Problem recipient is in DISABLED OU"
                                                $smsg += "`nAND/OR ADUser is DISABLED" ; 
                                                $smsg += "`n=>Improper offboard - no functional recipient in ExchangeOnline or Exchange Onprem" ;
                                                $smsg += "`n=>Offboard process created a mail loop" ; 
                                                $smsg += "`nADUser`n$(($adu | ft -a Enabled,DistinguishedName|out-string).trim())" ; 
                                                if ($logging) { Write-Log -LogContent $smsg -Path $logfile -useHost -Level WARN -Indent} 
                                                else{ write-WARNING "$((get-date).ToString('HH:mm:ss')):$($smsg)" } ; 
                                            } ; 
                                        } ; 
                            ...
                            if($aggDetailedOtherFails){
                                $smsg = "adding:`$hReports.MsgsFailOtherDetail" ; 
                                if ($logging) { Write-Log -LogContent $smsg -Path $logfile -useHost -Level Info } #Error|Warn|Debug 
                                else{ write-host -foregroundcolor yellow "$((get-date).ToString('HH:mm:ss')):$($smsg)" } ; 
                                $hReports.add('MsgsFailOtherDetail',$aggDetailedOtherFails) ; 
                            } ; 
                        } ; 
