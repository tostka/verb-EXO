...
function resolve-user {
    ...
    * 10:57 AM 2/11/2026 updated resolve-user to output detailed aduser.whencreated etc report for split brain misonboarding and output fix syntax.; fixed repair script name, added RemoteRoutingSuffix constant
    ...
    * 12:05 PM 7/14/2021 rem'd requires: verb-exo  rem'd requires version 5 (gen'ing 'version' is specified more than once.); rem'd the $rgxSamAcctName, gen's parsing errors compiling into mod ;  added alias 'ulu'; added mailcontact excl on init grcp, to force those to exombx qry ; init vers
    ...
    #>
        ...
        $RemoteRoutingSuffix = 'toroco.mail.onmicrosoft.com'
        ...
        # 8:55 AM 3/30/2026 add proxyAddresses for ADC conflcit lookups
        $propsADU = 'UserPrincipalName','DisplayName','GivenName','Surname','Title','Company','Department','PhysicalDeliveryOfficeName',
            ...
            'Description','Info','whenCreated','whenChanged','proxyAddresses'
        ...
        $propsMGUMgrL1 = @{Name='userPrincipalName';Expression={$_.userPrincipalName }},
            @{Name='mail';Expression={$_.mail}} 
        $propsMGUMgrL2 = @{Name='OpOU';Expression={ ($_.onPremisesDistinguishedName.split(',') | select -skip 1) -join ',' }} ;  
        # 10:37 AM 3/30/2026 conflict testing object details returned
        $prpmgUXTest = "id","displayName","userPrincipalName","proxyAddresses","OnPremisesSyncEnabled","OnPremisesLastSyncDateTime","OnPremisesDistinguishedName"  ;
        $prpadUXTest = "userprincipalname","samaccountname","distinguishedname","ObjectGUID","displayname"
        $sQot = [char]34 ;
        $sQotS = [char]39 ;
        ...
        if(-not (gi function:get-MgUserFull -ea 0)){
        ...
        if(-not (gi function:Convert-ADUserObjectGuidToImmutableID -ea 0)){
        ...
        if(-not (gi function:Convert-MGUserOnPremisesImmutableIdToADUserObjectGuid -ea 0)){
    ...
    PROCESS{
        ...
        foreach ($usr in $users){
            ...
            $hSum = [ordered]@{
                ...
                #IsAADDisabled = $false ; 
                IsMGDisabled = $false ; 
                isHardMatchMgAD = $false ; 
                isHardMatchAdMg = $FALSE ;
                isHybridExGuidMatch = $FALSE ;
                iSProvisioningErrors = $FALSE 
            } ;
            $procd++ ;
            ...
            if($pltgMGUsr.UserPrincipalName){
                ...
                if(-not($hSum.MGUser)){
                    ...
                    TRY{
                        ...
                        if($hSum.MGUser   +=  get-MgUserFull -userid $pltgMGUsr.UserPrincipalName){$hSum.MGUser = $hSum.MGUser | select -first $MaxRecips }
...
"@ ; 
            ...
            #region MGU_PROVISIONING_ERR ; #*------v MGU_PROVISIONING_ERR v------
            if($hSum.MGUser.OnPremisesProvisioningErrors){
                $hsum.iSProvisioningErrors = $true ; 
                $smsg = "`n`nMGUser.OnPremisesProvisioningErrors!" ; 
                if($hSum.MGUser.OnPremisesProvisioningErrors.IsResolved -ne $true){
                    $smsg += "`n**UNRESOLVED**! IsResolved:$($hSum.MGUser.OnPremisesProvisioningErrors.IsResolved)" ;
                }else{
                    $smsg += "`n**RESOLVED**! IsResolved:$($hSum.MGUser.OnPremisesProvisioningErrors.IsResolved)" ;
                }
                $sBnr4="`n#*``````v OnPremisesProvisioningErrors : v``````" ; 
                $smsg += $sBnr4 ; 
                $smsg += "`n`n$(($hSum.MGUser.OnPremisesProvisioningErrors|out-string).trim())`n`n" ; 
                $smsg += $sBnr4.replace('`v','`^').replace('v`','^`')
                $smsg += "`n`n" ;
                write-warning  $smsg
                # ATTEMPT CONFLICT OBJ QRYS
                #$prpmgUXTest = "id","displayName","userPrincipalName","proxyAddresses","OnPremisesSyncEnabled","OnPremisesLastSyncDateTime"  ;
                #$prpadUXTest = "userprincipalname","samaccountname","distinguishedname","ObjectGUID","displayname"
                if($hsum.MGUser.OnPremisesProvisioningErrors.category -eq 'PropertyConflict'){
                    switch -regex ($hsum.MGUser.OnPremisesProvisioningErrors.PropertyCausingError){
                        'UserPrincipalName|ProxyAddresses'{
                            # upn & proxy overlap: do both as a set - UPN may be a prox conflct
                            # UPN                            
                            #$fltr = "userPrincipalName eq '$($hsum.MGUser.Userprincipalname)'"            
                            #$fltr = "userPrincipalName eq '$($hsum.MGUser.OnPremisesProvisioningErrors.value)'" ; 
                            $fltr = "userPrincipalName eq '$( ($hsum.MGUser.OnPremisesProvisioningErrors.value) -replace 'smtp:','')'" ;
                            write-host -foregroundcolor yellow "Find mgUser conflicts on :`n$($fltr)" ; 
                            if($mgConflictMgUPN = Get-MgUser -All -Filter $fltr -Select $prpmgUXTest  | ?{$_.id -ne $hsum.MGUser.Id}){
                                if($hsum.keys -contains 'mgConflictMgUPN'){}else{$hsum.add('mgConflictMgUPN',$null) } ; 
                                $hsum.mgConflictMgUPN +=  @($mgConflictMgUPN| select -first $MaxRecips)                                  
                            }else{
                                write-verbose "No matches"
                            }
                            if($hsum.$mgConflictMgUPN){
                                $hsum.$mgConflictMgUPN = $hsum.$mgConflictMgUPN | select -unique ; 
                                foreach($mgC in $mgConflictMgUPN){               
                                    $smsg = "`n`n-->CONFLICTING UPN(s):`n$(($mgC| select ($prpmgUXTest | ?{$_ -notmatch 'proxy'}) | fl | out-string).trim())" ; 
                                    $smsg += "`n---proxyAddresses(s):`n$(($mgC| select -expand proxyAddresses | out-string).trim())`n`n" ; 
                                    write-warning $smsg ; 
                                } ; 
                            } ;                             
                            # PROXYADDRESSES 
                            <#ProxyAddresses takes 2 targete qrys: SMTP
                                Get-MgUser -All -Filter "proxyAddresses/any(p:p eq 'SMTP:jane.doe@contoso.com')" -Select "displayName,proxyAddresses"
                                #smtp:
                                Get-MgUser -All -Filter "proxyAddresses/any(p:p eq 'SMTP:Richard.Harris@toro.com')" -Select "displayName,proxyAddresses"
                                #>
                            $mgConflictMgProxy = @() ;                            
                            $thisProx = $hsum.MGUser.OnPremisesProvisioningErrors.value -replace 'smtp:','' ; 
                            foreach($type in @('SMTP','smtp')){
                                $fltr = "proxyAddresses/any(p:p eq '$($type):$($thisProx)')"    
                                write-host -foregroundcolor yellow "Find mgUser conflict on :`n$($fltr)" ;         
                                #if($mgConflictMgProxy = Get-MgUser -All -Filter $fltr -Select $prpmgUXTest){
                                # filter out the self obj
                                if($mgConflictMgProxy = Get-MgUser -All -Filter $fltr -Select $prpmgUXTest | ?{$_.id -ne $hsum.MGUser.Id}){
                                    if($hsum.keys -contains 'mgConflictMgProxy'){}else{$hsum.add('mgConflictMgProxy',$null) } ;                                     
                                    $hsum.mgConflictMgProxy += @($mgConflictMgProxy| select -first $MaxRecips)                                    
                                }else{
                                    write-verbose "No matches"
                                } ;                                 
                            } ; 
                            if($hsum.mgConflictMgProxy){
                                $hsum.mgConflictMgProxy = $hsum.mgConflictMgProxy | select -unique ; 
                                foreach($mgC in $hsum.mgConflictMgProxy){
                                    $smsg = "`n`n-->CONFLICTING mgUser.proxyAddresses(s):`n$(($mgC| select ($prpmgUXTest | ?{$_ -notmatch 'proxy'}) | fl | out-string).trim())" ; 
                                    $smsg += "`n---proxyAddresses(s):`n$(($mgC| select -expand proxyAddresses | out-string).trim())`n`n" ; 
                                    write-warning $smsg ; 
                                } ; 
                            } ; # cmpare to mgu
                            $smsg = "`n`n-->THIS MGUSER:`n$(($hsum.mguser| select ($prpmgUXTest | ?{$_ -notmatch 'proxy'}) | fl | out-string).trim())" ; 
                            $smsg += "`n---proxyAddresses(s):`n$(($hsum.mguser| select -expand proxyAddresses | out-string).trim())`n`n" ; 
                            write-warning $smsg ; 
                            # DO ADUSER.PROXY
                            $mgConflictAdProxy = @() ;                            
                            write-verbose "Find mgUser UPC conflict on current mgUser.UPN: $()" ; 
                            $thisProx = $hsum.MGUser.OnPremisesProvisioningErrors.value -replace 'smtp:','' ; 
                            foreach($type in @('SMTP','smtp')){
                                $fltr = "(proxyAddresses=$($type):$($thisProx))"
                                write-host -foregroundcolor yellow "Find ADUser conflict on :`n$($fltr)" ; 
                                #"proxyAddresses/any(p:p eq '$($type):$($thisProx)')"            
                                #if($mgConflictAdProxy = Get-MgUser -All -Filter $fltr -Select $prpmgUXTest){
                                # filter out the self obj
                                if($mgConflictAdProxy = Get-ADUser -LDAPFilter $fltr -Properties $prpadUXTest){
                                    # | ?{$_.id -ne $hsum.MGUser.Id}
                                    if($hsum.keys -contains 'mgConflictAdProxy'){}else{$hsum.add('mgConflictAdProxy',$null) } ; 
                                    $hsum.mgConflictAdProxy +=  @($mgConflictAdProxy| select -first $MaxRecips)                                     
                                }else{
                                    write-verbose "No matches"
                                } ; 
                            } ; 
                            if($hsum.mgConflictAdProxy){
                                $hsum.mgConflictAdProxy = $hsum.mgConflictAdProxy | select -unique ; 
                                foreach($adC in $hsum.mgConflictAdProxy){
                                    $smsg = "`n`n-->CONFLICTING ADUser.proxyAddress(s):`n$(($mgConflictAdProxy | select ($prpadUXTest|?{$_ -notmatch 'proxy'}) |fl | out-string).trim())" ; 
                                    $smsg += "`n---proxyAddresses(s):`n$(($mgConflictAdProxy | select -expand proxyAddresses |fl | out-string).trim())`n`n" ; 
                                    write-warning $smsg ; 
                                } ;
                            } ; 
                            # cmpare to adu
                            $smsg = "`n`n-->THIS ADUSER:`n$(($hsum.ADuser| select ($prpmgUXTest | ?{$_ -notmatch 'proxy'}) | fl | out-string).trim())" ; 
                            $smsg += "`n---proxyAddresses(s):`n$(($hsum.ADuser| select -expand proxyAddresses | out-string).trim())`n`n" ; 
                            write-warning $smsg ; 
                        }                         
                        default{
                            $smsg = "Unconfigured `$hsum.MGUser.OnPremisesProvisioningErrors.PropertyCausingError: $($hsum.MGUser.OnPremisesProvisioningErrors.PropertyCausingError)`n(no follow-up conflict qry run)" ; 
                            write-warning $smsg ; 
                        }
                    }
                } else {
                    $smsg = "Unconfigured `$hsum.MGUser.OnPremisesProvisioningErrors.category: $($hsum.MGUser.OnPremisesProvisioningErrors.category)`n(no follow-up conflict qry run)" ; 
                    write-warning $smsg ; 
                } ; 
            } ; 
            #endregion MGU_PROVISIONING_ERR ; #*------^ END MGU_PROVISIONING_ERR ^------

            #region CONFIRM_BIDIR_HARDMATCH ; #*------v CONFIRM_BIDIR_HARDMATCH v------
            if($hsum.aduser -AND $hsum.mguser){
                $hsum.isHardMatchMgAD = $false ; 
                $hsum.isHardMatchAdMg = $FALSE ;
                if( ($ImmutGuid = $hsum.mguser.onPremisesImmutableId | Convert-MGUserOnPremisesImmutableIdToADUserObjectGuid) -AND ($TestMatchADU = get-aduser -id $ImmutGuid) ){
                    if($TestMatchADU.objectguid -eq $hsum.aduser.objectguid){
                        $hsum.isHardMatchMgAD = $true ; 
                    }else {$hsum.isHardMatchMgAD = $false }
                } ; 
                if( ($MguOPImmut = $hsum.aduser.objectguid | Convert-ADUserObjectGuidToImmutableID) -AND ($TestMatchMGU = Get-MgUser -Filter "onPremisesImmutableId eq '$MguOPImmut'") ){
                    if($TestMatchMGU.id -eq $hsum.Mguser.id){
                        $hsum.isHardMatchAdMg = $true  ; 
                    }else {$hsum.isHardMatchAdMg = $false }                    
                }
                if($hsum.isHardMatchMgAD -AND $hsum.isHardMatchAdMg){                    
                    write-host -foregroundcolor green "ADUser.ObjectGuid *HARDMATCHES* MgUser.onPremisesImmutableId" ; 
                } else { 
                    $smsg = "ADUser.ObjectGuid *DOES NOT HARDMATCH* MgUser.onPremisesImmutableId!" ; 
                    $smsg += "`n`$hsum.isHardMatchAdMg:$($hsum.isHardMatchAdMg)`n`$hsum.isHardMatchMgAD:$($hsum.isHardMatchMgAD)" ;
                    WRITE-WARNING $SMSG ;
                }                
            } ; 
            #endregion CONFIRM_BIDIR_HARDMATCH ; #*------^ END CONFIRM_BIDIR_HARDMATCH ^------
            #$hsum.isExGuidMatch OPRemoteMailbox, xoMailbox
            #region CONFIRM_HYBRID_EXGUIDMATCH ; #*------v CONFIRM_HYBRID_EXGUIDMATCH v------
            if($hsum.OPRemoteMailbox -AND $hsum.xoMailbox){
                $hsum.isHybridExGuidMatch = $false ;                 
                if($hsum.OPRemoteMailbox.ExchangeGuid.guid -eq $hsum.xoMailbox.ExchangeGuid.guid){
                    $hsum.isHybridExGuidMatch = $true ; 
                    write-host -foregroundcolor green "HYBRID-ALIAGN:OPRemoteMailbox.ExchangeGuid -eq xoMailbox.ExchangeGuid" ; 
                }else{
                    $hsum.isHybridExGuidMatch = $false ;                 
                    WRITE-WARNING "HYBRID-ALIAGN:OPRemoteMailbox.ExchangeGuid *DOES NOT EQUAL* xoMailbox.ExchangeGuid" ; 
                }
            } ; 
            #endregion CONFIRM_HYBRID_EXGUIDMATCH ; #*------^ END CONFIRM_HYBRID_EXGUIDMATCH ^------
            ...
            if($hsum.IsSplitBrain){
                ...
                if($hsum.aduAge -gt $hsum.opMbxChangeAge){
                    ...
                    $smsg += "`n==>RUN:`nRepair-ExHybridSplitBrainMailbox.ps1 -UserPrincipalName $($hsum.aduser.userprincipalname) -RemoteRoutingSuffix 'toroco.mail.onmicrosoft.com' -CloudExchangeGuid '$($hsum.xomailbox.exchangeguid.guid)'" ;
